/**
 * Nội dung giải thích từng chỉ báo — port nguyên văn
 * `dashboard/src/features/strategy/indicatorInfo.ts`.
 *
 * Khoá là `indicator` của factor trong `GET /backtest/catalog` (cùng id với
 * `INDICATOR_DISPLAY` của backend-v2), nên chỉ báo mới thêm ở backend vẫn hiển
 * thị được trong thư viện; chỉ popover giải thích là không có.
 */
