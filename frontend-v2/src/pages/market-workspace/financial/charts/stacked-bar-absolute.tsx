import { colorVar, fmtNum, nonNeg, round } from "./chart-tokens"

export interface StackedPart {
  label: string
  value: number
  /** semantic colour key (e.g. "fill-g" | "g" | "green") — resolved to a token */
  cls: string
}
...
/**
 * KHỐI 3 — cột chồng TUYỆT ĐỐI 5 năm (số tiền thật, có trục y).
 * Mỗi năm = 1 cột = tổng, chia thành các dải xếp chồng. Negative part values
 * are clamped to 0 so no bar renders inverted.
 */
export function StackedBarAbsolute({
  ...
  return (
    ...
    >
      {/* gridlines + y labels */}
      <g className="fill-muted-foreground text-xs tabular-nums" textAnchor="end">
        {ticks.map((t, i) => (
        ...
        return (
          <g key={`col-${s.year}`}>
            {s.parts.map((p, j) => {
              ...
              cursor = y
              const pctShare = total > 0 ? Math.round((nonNeg(p.value) / total) * 100) : 0
              return (
                <g key={`seg-${j}`}>
                  <rect x={x} y={y} width={COL_W} height={h} fill={colorVar(p.cls)} />
                  {showPct && h > 16 ? (
                    <text
                      className="fill-primary-foreground text-xs font-semibold tabular-nums"
                      x={cx}
                      y={segCenter + 4}
                      textAnchor="middle"
                    >
      ...
      <g className="fill-muted-foreground text-xs" textAnchor="middle">
        {series.map((s, i) => (
          <text key={`xl-${s.year}`} x={round(colX(i) + COL_W / 2)} y={BASE_Y + 20}>
            {s.year}
          </text>
        ))}
      </g>
