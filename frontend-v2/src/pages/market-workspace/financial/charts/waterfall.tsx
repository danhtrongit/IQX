...
function kindToken(k: WaterfallKind): string {
  if (isTotal(k)) return "var(--chart-1)"
  if (isSubtract(k)) return "var(--chart-4)"
  return "var(--chart-3)"
}
function resolveBands(steps: WaterfallStep[]): { lo: number; hi: number }[] {
  let cumulative = 0
  const bands: { lo: number; hi: number }[] = []
  for (const step of steps) {
    const value = Math.abs(step.value)
    if (isTotal(step.kind)) {
      cumulative = step.value
      bands.push({ lo: Math.min(0, step.value), hi: Math.max(0, step.value) })
    } else if (isSubtract(step.kind)) {
      const hi = cumulative
      cumulative -= value
      bands.push({ lo: cumulative, hi })
    } else {
      const lo = cumulative
      cumulative += value
      bands.push({ lo, hi: cumulative })
    }
  }
  return bands
}
...
export function Waterfall({ steps, ariaLabel = "Cầu nối dòng tiền" }: WaterfallProps) {
  ...
  const bands = resolveBands(steps)
  ...
  return (
    ...
    >
      ...
      {steps.map((s, i) => {
        ...
        return (
          <g key={`step-${i}`}>
            <rect x={x} y={yTop} width={round(barW)} height={h} rx={3} fill={kindToken(s.kind)} />
            <text
            ...
            <text className="fill-muted-foreground text-xs" x={cx} y={LABEL_Y} textAnchor="middle">
              {s.label}
            </text>
          </g>
        )
