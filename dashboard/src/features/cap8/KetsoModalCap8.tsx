 ...
 * Cấp 8 không đi qua màn Săn mã. Một `huntFilter: null` cứng KHÔNG phải "chưa
 * biết" — `coachTemplateCap5` đọc nó thành câu khẳng định "mã này KHÔNG đến từ săn
 * mã". Thà VẮNG đoạn coach còn hơn có một đoạn nói sai.
 *
...
export function KetsoModalCap8({
  ...
  const coach = composeCoachCap8(
    ...
    cap3Situation,
    cap4Situation,
    // Cấp 5 mới = SĂN MÃ, và trang Cấp 8 không có nguồn `huntFilter` của lệnh →
  ...
  const buildRecord = (): Cap7TradeRecord => ({
    ...
    // trống chờ điền — trang Cấp 8 không có màn Săn mã nên lệnh Cấp 8 thật sự
