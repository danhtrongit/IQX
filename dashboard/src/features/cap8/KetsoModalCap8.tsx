 ...
 *    CẢ 8 đoạn text đều do module cấp dưới sinh ra, Cấp 8 chỉ thêm đoạn 8.
 *  - Khối phân loại: `PhanLoai4O` của Cấp 5 NGUYÊN VẸN; modal này chỉ sở hữu CỔNG
 *    + `POST /cap5/ketso`.
 ...
 *
 * ★ **CỔNG CỦA CẤP 5 KHÔNG BỊ NỚI** (spec §6 "cộng dồn"): `Đóng kết sổ ✓` vẫn khoá
 * tới khi `PhanLoai4O` báo đã chốt verdict, modal vẫn `closable={false}`, và LỐI RA
 * khi `GET /cap5/verdict` lỗi vẫn còn. Khối Kiểm tra danh mục của Cấp 8 KHÔNG thêm
 * cổng nào — spec §9 nói thẳng Cấp 8 là cảnh báo MỀM, không cổng cứng ở bất kỳ đâu.
 *
...
function hasViPham(flags: KetsoDataCap8["flags"]): boolean {
  return Boolean(
...
export function KetsoModalCap8({
  ...
  /**
   * ★ Lối ra BẮT BUỘC, kế thừa nguyên vẹn từ Cấp 5/6/7: modal `closable={false}` +
   * cổng fail-closed, nên nếu `GET /cap5/verdict` lỗi thì user sẽ kẹt. Khi (và chỉ
   * khi) query verdict LỖI: ghi 7 cờ kỷ luật Cấp 2, ghi nhật ký với ô 4 = NULL
   * (nhưng GIỮ các khối Đối chiếu / Đọc sổ lệnh / Kiểm tra danh mục — chúng độc lập
   * với verdict), rồi đóng. Đang tải KHÔNG hiện lối ra.
   */
  ...
  return (
    ...
    >
      ...
      {/* ── PHÂN LOẠI 4 Ô (giữ nguyên Cấp 5) ──────────────────────────────── */}
      ...
      {/* Lớp coach 5 — Cấp 5 (quyết định vs kết quả), giữ nguyên. */}
      {coach.cap5 && (
        <div
      ...
      {/* CỔNG (kế thừa Cấp 5 §4): khoá tới khi verdict được chốt. */}
      <button
        type="button"
      ...
      {!verdictUser && !escapeVisible && (
        <p className="cap5-ketso-gate-note" data-testid="cap8-ketso-gate-note">
      ...
      {escapeVisible && (
        <>
          <button
            ...
            disabled={closing}
            data-testid="cap8-ketso-escape"
          >
            Đóng kết sổ — chưa phân loại được
          </button>
          <p className="cap5-ketso-escape-note" data-testid="cap8-ketso-escape-note">
            ...
            Khối Đối chiếu, khối Đọc sổ lệnh và khối Kiểm tra danh mục ở trên vẫn được ghi. Bạn
            không bị kẹt ở đây: đóng lại và thử ở lệnh sau.
          </p>
        </>
      )}
    </Modal>
