...
interface BackendEpisodeBrief {
  ...
  title: string
  description: string | null
  content_type: EpisodeBrief["contentType"]
...
interface BackendEpisodeContent {
  ...
  title: string
  description: string | null
  content_type: EpisodeContent["contentType"]
...
export const lessonsApi = {
  ...
  getCourse: async (slug: string): Promise<CourseDetail> => {
    const raw = await api.get(`lessons/courses/${slug}`).json<BackendCourseDetail>()
    return adaptCourseDetail(raw)
  },

  getEpisodeContent: async (id: string): Promise<EpisodeContent> => {
    const raw = await api.get(`lessons/episodes/${id}/content`).json<BackendEpisodeContent>()
    return adaptEpisodeContent(raw)
  },

  saveProgress: async (id: string, payload: SaveProgressPayload): Promise<void> => {
