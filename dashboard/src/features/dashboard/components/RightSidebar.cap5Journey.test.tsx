...
/**
 * Cấp 5 Task FE3 — `RightSidebar`'s "journey"/"cap5-analysis" panel routing must
...
describe("RightSidebar — Cấp 5 journey/analysis panel routing (Cấp 5 Task FE3)", () => {
  it('resolves "journey" to JourneyPanelCap5 while isCap5Active', () => {
  ...
  it('resolves "journey" to JourneyPanelCap4 when isCap5Active is false (regression check)', () => {
    useCap1EventsMock.mockReturnValue({ isCap1Active: true })
    useCap2EventsMock.mockReturnValue({ isCap2Active: true })
    useCap3EventsMock.mockReturnValue({ isCap3Active: true })
    useCap4EventsMock.mockReturnValue({ isCap4Active: true })
  ...
  it('resolves "cap5-analysis" to Cap5PortfolioAnalysisPanel while isCap5Active', () => {
    useCap5EventsMock.mockReturnValue({ isCap5Active: true })
    render(
    ...
    expect(screen.getByTestId("cap5-analysis-panel")).toBeInTheDocument()
  })

  it('falls back to Cấp 0\'s JourneyPanel for "cap5-analysis" when isCap5Active is false (defensive)', () => {
    ...
    expect(screen.queryByTestId("cap5-analysis-panel")).not.toBeInTheDocument()
  })

  it('the mobile header label for "cap5-analysis" is "Phân tích danh mục"', () => {
    useCap5EventsMock.mockReturnValue({ isCap5Active: true })
    render(
      <SidebarProvider defaultPanel="cap5-analysis">
        <RightSidebar />
