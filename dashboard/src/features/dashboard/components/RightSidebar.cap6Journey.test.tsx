...
/**
 * Cấp 6 Task FE3 — `RightSidebar`'s "journey"/"cap6-analysis" panel routing must
 * pick the Cấp 6 views (`JourneyPanelCap6`/`Cap6PortfolioAnalysisPanel`) while
 * `isCap6Active`, and fall through to Cấp 5's/…/Cấp 0's own routing otherwise —
 * mirrors `RightSidebar.cap5Journey.test.tsx`, one level up. A real Cấp 6 session
 * has `isCap1Active`…`isCap5Active` true too (Cấp 6 wraps all six providers —
 * see `Cap6TradingPage`), so "journey" must check `isCap6Active` FIRST.
 */
...
describe("RightSidebar — Cấp 6 journey/analysis panel routing (Cấp 6 Task FE3)", () => {
  it('resolves "journey" to JourneyPanelCap6 while isCap6Active', () => {
  ...
  it('resolves "journey" to JourneyPanelCap5 when isCap6Active is false (regression check)', () => {
    useCap1EventsMock.mockReturnValue({ isCap1Active: true })
    useCap2EventsMock.mockReturnValue({ isCap2Active: true })
    useCap3EventsMock.mockReturnValue({ isCap3Active: true })
    useCap4EventsMock.mockReturnValue({ isCap4Active: true })
    ...
    expect(screen.getByTestId("journey-panel-cap5")).toBeInTheDocument()
    expect(screen.queryByTestId("journey-panel-cap6")).not.toBeInTheDocument()
  })

  it('resolves "cap6-analysis" to Cap6PortfolioAnalysisPanel while isCap6Active', () => {
    useCap6EventsMock.mockReturnValue({ isCap6Active: true })
    render(
    ...
    expect(screen.getByTestId("cap6-analysis-panel")).toBeInTheDocument()
  })

  it('falls back to Cấp 0\'s JourneyPanel for "cap6-analysis" when isCap6Active is false (defensive)', () => {
    ...
    expect(screen.queryByTestId("cap6-analysis-panel")).not.toBeInTheDocument()
  })

  it('the mobile header label for "cap6-analysis" is "Phân tích danh mục"', () => {
    useCap6EventsMock.mockReturnValue({ isCap6Active: true })
    render(
      <SidebarProvider defaultPanel="cap6-analysis">
        <RightSidebar />
