...
function BotToolbarEntry() {
  const { data: progress } = useCap6Progress(true)
  const { activePanel, setActivePanel } = useSidebar()
  if (!progress?.graduated_at) return null
  return <ToolbarButton item={{ icon: IconStar, label: "Bot của tôi", id: "bot", panel: "bot" }}
    isActive={activePanel === "bot"} onClick={() => setActivePanel("bot")} />
}
...
export function RightToolbar({
  ...
  // ★ Nút «Săn mã» của Cấp 5 chỉ mọc ra TRONG
  ...
  const ITEMS: ToolbarItem[] = [
    ...
    ...((isCap0Active || isCap1Active) ? [{ icon: IconStar, label: "Linh thú", id: "identity", panel: "identity" } as ToolbarItem] : []),
  ...
  return (
    <aside
      ...
      className={cn("fixed bottom-0 left-0 right-0 z-50 w-full h-[52px] bg-[var(--color-bg-2)] border-t border-[var(--color-border-2)] flex flex-row items-center justify-around px-2 pb-[env(safe-area-inset-bottom)] md:static md:w-20 md:h-full md:flex-col md:border-l md:border-t-0 md:py-1 md:px-0.5 gap-0.5", (isCap0Active || isCap1Active) && "journey-toolbar")}
    >
      {ITEMS.map((item) => (
        <ToolbarButton
          key={item.id}
