 ...
 *
 * Adapter over the backend market-data API (OHLCV + intraday + symbol search +
 * AI news marks). Ported from dashboard-bak; all network access now routes
 * through the shared `api` ky client (prefixUrl `/api/v1`, auth + 401 refresh
 * baked in) instead of raw fetch.
 */
...
async function fetchBars(
  ...
  try {
    // GET market-data/quotes/{symbol}/ohlcv?interval=1D&start=YYYY-MM-DD&end=YYYY-MM-DD
    const searchParams: Record<string, string> = { interval }
    if (from) searchParams.start = formatISODate(new Date(clampTs(from) * 1000))
    if (to) searchParams.end = formatISODate(new Date(clampTs(to) * 1000))

    const json = await api
      .get(`market-data/quotes/${symbol.toUpperCase()}/ohlcv`, { searchParams })
      .json<{ data?: unknown } | unknown>()

    const items =
      (json as { data?: unknown })?.data ?? (Array.isArray(json) ? json : [])
    ...
    const bars: Bar[] = (Array.isArray(items) ? items : [])
      .map((item: any) => {
        ...
        if (typeof raw === "number") {
          ...
          // Guard: if value already in ms (> year 2100 in seconds) keep as-is.
...
function sentimentToColor(s: "positive" | "negative" | "neutral"): {
  border: string
  background: string
} {
  switch (s) {
    case "positive":
      return { border: "#10b981", background: "#10b981" }
    case "negative":
      return { border: "#ef4444", background: "#ef4444" }
    default:
      return { border: "#f59e0b", background: "#f59e0b" }
  }
}
  ...
  private async _poll() {
    for (const [, sub] of this._subscribers) {
      try {
        ...
        const from = sub.lastBar
          ? Math.floor(sub.lastBar.time / 1000) - 60
          : now - 86400
...
export function createDataFeed(): any {
  ...
  return {
    ...
    async searchSymbols(
      ...
      try {
        // GET market-data/reference/symbols/search?q=…&page_size=…&include_indices=true
        const json = await api
