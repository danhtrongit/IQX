/**
 * Theme-aware pattern illustrations rendered with plain SVG. Ported from
 * `dashboard-bak/src/components/patterns/pattern-illustration.tsx`.
 *
 * `CandlePatternIllustration` renders a sequence of candles for a TA-Lib candle
 * pattern (Doji, Hammer, Engulfing, ...). `ChartPatternIllustration` renders the
 * classical chart patterns (Ascending Triangle, Head & Shoulders, ...) with
 * reference lines for neckline / support / resistance.
 */
...
function CandleSVG({ data, width = 320, height = 200, caption }: ChartProps) {
  ...
  for (const c of candles) allValues.push(c.h, c.l, c.o, c.c)
  for (const r of refs) {
  ...
  return (
    ...
    >
      ...
      {/* Caption */}
      {caption && (
        <text x={width / 2} y={height - 8} textAnchor="middle" fontSize={9} fill="var(--color-text-3)">
          {caption}
        </text>
      )}
    </svg>
...
function buildCandlePattern(name: string, signal: Signal): PatternData {
  switch (name) {
    case "Doji": {
      ...
      return {
        candles: [...lead, { o: last + 4, h: last + 7, l: last + 1, c: last + 4 }],
    ...
    case "Belt-hold": {
      ...
      const sig =
        signal === "bullish"
          ? { o: last - 6, h: last + 8, l: last - 6, c: last + 8 }
          : { o: last + 6, h: last + 6, l: last - 8, c: last - 8 }
  ...
  if (signal === "bullish") {
    const lead = downtrend(4, 90, 5)
    const last = lead[lead.length - 1].c
    return { candles: [...lead, { o: last - 1, h: last + 8, l: last - 2, c: last + 7 }] }
  }
  if (signal === "bearish") {
    const lead = uptrend(4, 60, 5)
    const last = lead[lead.length - 1].c
    return { candles: [...lead, { o: last + 1, h: last + 2, l: last - 8, c: last - 7 }] }
  }
  return {
    candles: [
      { o: 70, h: 76, l: 68, c: 73 },
...
export function ChartPatternIllustration({ name, caption }: { name: string; caption?: string }) {
