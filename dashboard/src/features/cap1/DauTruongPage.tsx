...
import { Cap0TradingPage, useCap0Progress, usePlacementStatus } from "@/features/cap0"
...
function ProgressUnavailable({ retry }: { retry: () => unknown }) {
  return (
    <div role="alert" className="flex h-svh flex-col items-center justify-center gap-3 bg-[var(--color-bg-1)] p-6 text-center">
      <p>Chưa tải được tiến trình hành trình. Vui lòng thử lại.</p>
      <Button type="primary" onClick={() => { void retry() }}>Thử lại</Button>
    </div>
  )
}
 ...
 * current level program (Cấp 0 … Cấp 6). Picks the flow by the
 ...
 * xuống cấp dưới) cho tới khi trần được nâng. Hiện `CAP_MAX_ENABLED = 6`: tốt nghiệp Cấp 6 hoàn thành lộ trình.
 ...
 *    arrival — same pattern, one more level up.
 * Historical branches below are disabled by the current ceiling.
...
export function DauTruongPage() {
  ...
  const { data: cap0Progress, isFetched: cap0Fetched, isError: cap0Error, refetch: retryCap0 } = useCap0Progress(isAuthenticated)
  const cap0Graduated = !!cap0Progress?.graduated_at
  const { data: placement, isFetched: placementFetched, isError: placementError, refetch: retryPlacement } = usePlacementStatus(isAuthenticated && !cap0Graduated)
  const placedLevel = placement?.placed_level ?? 0
  const canEnterCap1 = cap0Graduated || placedLevel >= 1

  // Only query Cấp 1 progress once Cấp 0 is confirmed graduated — no wasted
  // request for users who haven't gotten there yet.
  const shouldQueryCap1 = isAuthenticated && canEnterCap1
  const { data: cap1Progress, isFetched: cap1Fetched, isError: cap1Error, refetch: retryCap1 } = useCap1Progress(shouldQueryCap1)
  const enterCap1 = useEnterCap1()
  const enterCap1AttemptedRef = useRef(false)

  useEffect(() => {
    if (!shouldQueryCap1 || !cap1Fetched || cap1Error) return
    if (placedLevel >= 2) return
  ...
  }, [shouldQueryCap1, cap1Fetched, cap1Error, cap1Progress, placedLevel])
  ...
  const progressPastCap1 = capOpen(2) && (cap1Graduated || placedLevel >= 2)
  ...
  const { data: cap2Progress, isFetched: cap2Fetched, isError: cap2Error, refetch: retryCap2 } = useCap2Progress(shouldQueryCap2)
  const enterCap2 = useEnterCap2()
  const enterCap2AttemptedRef = useRef(false)

  useEffect(() => {
    if (!shouldQueryCap2 || !cap2Fetched || cap2Error) return
    ...
    enterCap2.mutate()
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [shouldQueryCap2, cap2Fetched, cap2Error, cap2Progress])
  ...
  const { data: cap3Progress, isFetched: cap3Fetched, isError: cap3Error, refetch: retryCap3 } = useCap3Progress(shouldQueryCap3)
  const enterCap3 = useEnterCap3()
  const enterCap3AttemptedRef = useRef(false)

  useEffect(() => {
    if (!shouldQueryCap3 || !cap3Fetched || cap3Error) return
    ...
    enterCap3.mutate()
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [shouldQueryCap3, cap3Fetched, cap3Error, cap3Progress])
  ...
  const { data: cap4Progress, isFetched: cap4Fetched, isError: cap4Error, refetch: retryCap4 } = useCap4Progress(shouldQueryCap4)
  const enterCap4 = useEnterCap4()
  const enterCap4AttemptedRef = useRef(false)

  useEffect(() => {
    if (!shouldQueryCap4 || !cap4Fetched || cap4Error) return
    ...
    enterCap4.mutate()
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [shouldQueryCap4, cap4Fetched, cap4Error, cap4Progress])
  ...
  const { data: cap5Progress, isFetched: cap5Fetched, isError: cap5Error, refetch: retryCap5 } = useCap5Progress(shouldQueryCap5)
  const enterCap5 = useEnterCap5()
  const enterCap5AttemptedRef = useRef(false)

  useEffect(() => {
    if (!shouldQueryCap5 || !cap5Fetched || cap5Error) return
    ...
    enterCap5.mutate()
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [shouldQueryCap5, cap5Fetched, cap5Error, cap5Progress])
  ...
  const { data: cap6Progress, isFetched: cap6Fetched, isError: cap6Error, refetch: retryCap6 } = useCap6Progress(shouldQueryCap6)
  const enterCap6 = useEnterCap6()
  const enterCap6AttemptedRef = useRef(false)

  useEffect(() => {
    if (!shouldQueryCap6 || !cap6Fetched || cap6Error) return
    ...
    enterCap6.mutate()
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [shouldQueryCap6, cap6Fetched, cap6Error, cap6Progress])
  ...
  const { data: cap7Progress, isFetched: cap7Fetched, isError: cap7Error, refetch: retryCap7 } = useCap7Progress(shouldQueryCap7)
  const enterCap7 = useEnterCap7()
  const enterCap7AttemptedRef = useRef(false)

  useEffect(() => {
    if (!shouldQueryCap7 || !cap7Fetched || cap7Error) return
    ...
    enterCap7.mutate()
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [shouldQueryCap7, cap7Fetched, cap7Error, cap7Progress])
  ...
  const { data: cap8Progress, isFetched: cap8Fetched, isError: cap8Error, refetch: retryCap8 } = useCap8Progress(shouldQueryCap8)
  const enterCap8 = useEnterCap8()
  const enterCap8AttemptedRef = useRef(false)

  useEffect(() => {
    if (!shouldQueryCap8 || !cap8Fetched || cap8Error) return
    ...
    enterCap8.mutate()
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [shouldQueryCap8, cap8Fetched, cap8Error, cap8Progress])
  ...
  if (!isAuthenticated) return wasAuthenticated ? <SessionExpired /> : <Cap0TradingPage />
  if (cap0Error && cap0Progress === undefined) return <ProgressUnavailable retry={retryCap0} />
  if (!cap0Fetched) return <FullPageSpinner />
  if (!cap0Graduated && placementError && placement === undefined) return <ProgressUnavailable retry={retryPlacement} />
  if (!cap0Graduated && !placementFetched) return <FullPageSpinner />
  if (!canEnterCap1) return <Cap0TradingPage />
  if (cap1Error && cap1Progress === undefined) return <ProgressUnavailable retry={retryCap1} />
  ...
  // tới khi trần được nâng. Với trần hiện tại (6), Cấp 6 là điểm dừng — và tab
  ...
  if (cap2Error && cap2Progress === undefined) return <ProgressUnavailable retry={retryCap2} />
  if (!cap2Fetched) return <FullPageSpinner />
  if (!capOpen(3) || !cap2Graduated) return <Cap2TradingPage />
  if (cap3Error && cap3Progress === undefined) return <ProgressUnavailable retry={retryCap3} />
  if (!cap3Fetched) return <FullPageSpinner />
  if (!capOpen(4) || !cap3Graduated) return <Cap3TradingPage />
  if (cap4Error && cap4Progress === undefined) return <ProgressUnavailable retry={retryCap4} />
  if (!cap4Fetched) return <FullPageSpinner />
  if (!capOpen(5) || !cap4Graduated) return <Cap4TradingPage />
  if (cap5Error && cap5Progress === undefined) return <ProgressUnavailable retry={retryCap5} />
  if (!cap5Fetched) return <FullPageSpinner />
  if (!capOpen(6) || !cap5Graduated) return <Cap5TradingPage />
  if (cap6Error && cap6Progress === undefined) return <ProgressUnavailable retry={retryCap6} />
  if (!cap6Fetched) return <FullPageSpinner />
  if (!capOpen(7) || !cap6Graduated) return <Cap6TradingPage />
  if (cap7Error && cap7Progress === undefined) return <ProgressUnavailable retry={retryCap7} />
  if (!cap7Fetched) return <FullPageSpinner />
  if (!capOpen(8) || !cap7Graduated) return <Cap7TradingPage />
  if (cap8Error && cap8Progress === undefined) return <ProgressUnavailable retry={retryCap8} />
