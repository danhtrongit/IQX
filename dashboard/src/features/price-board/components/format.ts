...
/**
 * Bảng giá formatters + color helpers (en-US locale everywhere).
...
export function fmtBil(n: number | null | undefined): string {
  if (!n) return "—"
  if (n >= 1e9) return (n / 1e9).toLocaleString("en-US", { maximumFractionDigits: 1 })
  if (n >= 1e6) return (n / 1e6).toLocaleString("en-US", { maximumFractionDigits: 2 })
  return Math.round(n).toLocaleString("en-US")
}
...
export function fmtTrieu(n: number | null | undefined): string {
  if (!n) return "—"
  return (n / 1e6).toLocaleString("en-US", { maximumFractionDigits: 1 })
}
