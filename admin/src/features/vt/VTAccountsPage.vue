...
const columns: DataTableColumns<VTAccountRow> = [
  { title: "User", key: "userEmail", render: (row) => h(RouterLink, { to: `/vt/accounts/${row.id}` }, { default: () => row.userEmail ?? row.userId }) },
  { title: "Tên", key: "userName" },
  { title: "Trạng thái", key: "status", render: (row) => h(NSpace, { size: 4 }, { default: () => [h(StatusTag, { status: row.status }), row.frozenAt ? h(StatusTag, { status: "pending", label: "Frozen" }) : null] }) },
  { title: "Cash", key: "cashAvailableVnd", render: (row) => fmtVnd(row.cashAvailableVnd) },
  { title: "Reserved", key: "cashReservedVnd", render: (row) => fmtVnd(row.cashReservedVnd) },
  { title: "Activated", key: "activatedAt", render: (row) => row.activatedAt ? fmtDateTime(row.activatedAt) : "-" },
...
<template><div class="page-stack"><div class="page-header"><div><h1 class="page-title">Tài khoản giao dịch ảo</h1><p class="page-subtitle">{{ page.total.value }} tài khoản</p></div></div><n-card><n-space wrap><n-input v-model:value="searchText" clearable placeholder="Email, tên" style="width:260px" @keyup.enter="search" /><n-select v-model:value="status" clearable :options="['active','suspended'].map((value)=>({label:value,value}))" placeholder="Trạng thái" style="width:160px" /><n-select v-model:value="frozenFilter" clearable :options="[{label:'Frozen only',value:'frozen'},{label:'All',value:'all'}]" placeholder="Frozen" style="width:160px" /><n-button type="primary" @click="search">Lọc</n-button></n-space></n-card><ErrorState v-if="page.error.value" :message="page.error.value" @retry="page.load" /><RemoteDataTable :columns="columns" :rows="page.rows.value" :loading="page.loading.value" :page="page.params.page" :page-size="page.params.pageSize" :item-count="page.total.value" @page-change="page.setPage" @page-size-change="page.setPageSize" /></div></template>
