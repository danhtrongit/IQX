  ...
  <div class="page-stack">
    ...
    <n-card v-if="user360" :loading="loading">
      <n-descriptions bordered :column="2">
        ...
        <n-descriptions-item label="Vai trò"><StatusTag :status="user360.user.role" :label="user360.user.role" /></n-descriptions-item>
        <n-descriptions-item label="Trạng thái"><StatusTag :status="user360.user.status" /></n-descriptions-item>
        <n-descriptions-item label="Trial used">{{ user360.trialUsed ? 'Có' : 'Không' }}</n-descriptions-item>
    ...
    <n-tabs v-if="user360" type="line" :animated="false">
      ...
      <n-tab-pane name="login" tab="Login history"><n-data-table :columns="loginColumns" :data="user360.loginHistory" /></n-tab-pane>
    </n-tabs>
  </div>
</template>
