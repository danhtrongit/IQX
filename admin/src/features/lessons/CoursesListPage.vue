...
import { labelForCourseLevel, labelForPublishState } from "@/lib/labels"
...
const levelOptions = ["beginner", "intermediate", "advanced"].map((value) => ({ label: labelForCourseLevel(value), value }))
const publishOptions = ["published", "draft"].map((value) => ({ label: labelForPublishState(value), value }))
const columns: DataTableColumns<CourseRow> = [
  ...
  { title: "Cấp độ", key: "level", render: (row) => labelForCourseLevel(row.level) },
  { title: "Premium", key: "isPremium", render: (row) => h(StatusTag, { status: row.isPremium, label: row.isPremium ? "Premium" : "Miễn phí" }) },
  { title: "Xuất bản", key: "isPublished", render: (row) => h(StatusTag, { status: row.isPublished, label: row.isPublished ? "Đã xuất bản" : "Bản nháp" }) },
  { title: "Bài học", key: "totalEpisodes" },
