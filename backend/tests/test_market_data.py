...
# ══════════════════════════════════════════════════════
# VCI gap-chart nhiều mã (``fetch_ohlcv_multi``) — nguồn nến của Cấp 5 «Săn mã»
# ══════════════════════════════════════════════════════
# ★★ 75 dòng fetcher này trước đây KHÔNG có test nào trong repo. Nó là nguồn
# nến cho máy săn mã quét cả sàn HOSE, và cái GUARD quan trọng nhất của nó —
# "thà BỎ chart chứ không gán nến mã này cho mã khác" — có thể bị xoá âm thầm:
# gán lệch nến là một danh sách "top 10 mã" nói về những mã khác hẳn.


def _chart(symbol: str | None, *, close: float, n: int = 3) -> dict:
    chart: dict[str, Any] = {
        "t": [1_780_000_000 + i * 86_400 for i in range(n)],
        "o": [close] * n,
        "h": [close] * n,
        "l": [close] * n,
        "c": [close] * n,
        "v": [1_000.0] * n,
        "accumulatedValue": [1_200.0] * n,
    }
...
@pytest.mark.asyncio
async def test_ohlcv_multi_ghep_theo_truong_symbol_khong_theo_thu_tu():
    """Ghép theo trường ``symbol`` upstream trả về, KHÔNG theo thứ tự request.

    Upstream đảo thứ tự chart là chuyện bình thường; ghép theo vị trí khi đã có
    tên mã là cách gán nến của AAA cho BBB.
    """
    from app.services.market_data.sources import vietcap

    with patch(
        "app.services.market_data.sources.vietcap.fetch_json", new_callable=AsyncMock
    ) as mock:
        mock.return_value = [_chart("BBB", close=20.0), _chart("AAA", close=10.0)]
        out, _url = await vietcap.fetch_ohlcv_multi(
            ["AAA", "BBB"], end_ts=1_780_300_000, count_back=31
        )

    assert set(out) == {"AAA", "BBB"}
    assert out["AAA"][0]["close"] == 10.0
    assert out["BBB"][0]["close"] == 20.0
    # Payload phải mang đúng lô mã + countBack đã xin.
    payload = mock.await_args.kwargs["json_body"]
    assert payload["symbols"] == ["AAA", "BBB"]
    assert payload["countBack"] == 31


@pytest.mark.asyncio
async def test_ohlcv_multi_chart_khong_ten_chi_ghep_theo_vi_tri_khi_du_so_luong():
    """Chart không có tên mã: chỉ được ghép theo vị trí khi SỐ LƯỢNG khớp hẳn."""
    from app.services.market_data.sources import vietcap

    with patch(
        "app.services.market_data.sources.vietcap.fetch_json", new_callable=AsyncMock
    ) as mock:
        mock.return_value = [_chart(None, close=10.0), _chart(None, close=20.0)]
        out, _url = await vietcap.fetch_ohlcv_multi(
            ["AAA", "BBB"], end_ts=1_780_300_000
        )
    assert out["AAA"][0]["close"] == 10.0
    assert out["BBB"][0]["close"] == 20.0
...
@pytest.mark.asyncio
async def test_ohlcv_multi_tha_bo_chart_chu_khong_gan_nen_lech_ma():
    """★★ GUARD: thiếu tên mã VÀ số lượng KHÔNG khớp ⇒ BỎ chart.

    Xoá guard này (ghép theo vị trí bất chấp) là gán nến của một mã cho ticker
    của mã khác — và cả màn Săn mã sẽ nói về những mã khác hẳn, không cách nào
    phát hiện từ giao diện.
    """
    from app.services.market_data.sources import vietcap
    ...
    with patch(
        ...
        # Xin 3 mã, upstream trả 2 chart vô danh ⇒ không biết chart nào của ai.
        mock.return_value = [_chart(None, close=10.0), _chart(None, close=20.0)]
        out, _url = await vietcap.fetch_ohlcv_multi(
            ["AAA", "BBB", "CCC"], end_ts=1_780_300_000
        )
    assert out == {}, "gán nến lệch mã còn tệ hơn không có nến"


@pytest.mark.asyncio
async def test_ohlcv_multi_chart_rong_va_data_la_khong_lam_no():
    """Chart không có nến nào thì KHÔNG vào kết quả (không tạo list rỗng giả), và
    response không phải list thì trả dict rỗng thay vì nổ."""
    from app.services.market_data.sources import vietcap

    with patch(
        "app.services.market_data.sources.vietcap.fetch_json", new_callable=AsyncMock
    ) as mock:
        mock.return_value = [{"symbol": "AAA", "t": [], "c": []}]
        out, _url = await vietcap.fetch_ohlcv_multi(["AAA"], end_ts=1_780_300_000)
    assert out == {}
