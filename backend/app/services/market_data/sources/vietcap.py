...
async def fetch_ohlcv_multi(
    ...
    *,
    ...
    """Fetch OHLCV for MANY symbols in one gap-chart call.

    The upstream endpoint already takes a ``symbols`` array; ``fetch_ohlcv``
    just never used more than one. Used by Cấp 5 «Săn mã», which has to screen
    the whole HOSE board and cannot afford one HTTP round trip per symbol.

    ★ Symbols are matched by the ``symbol`` field the upstream returns on each
    chart. If a chart carries no symbol we fall back to positional matching
    ONLY when the response length equals the request length — otherwise the
    charts are DROPPED rather than risk attaching one stock's bars to another
    stock's ticker.
    """
    url = f"{_TRADING_BASE}/chart/OHLCChart/gap-chart"
    headers = get_headers(_SOURCE)
    ...
    charts = [c for c in data if isinstance(c, dict)] if isinstance(data, list) else []
    positional = len(charts) == len(wanted)

    out: dict[str, list[dict[str, Any]]] = {}
    for idx, chart in enumerate(charts):
        raw_symbol = chart.get("symbol") or chart.get("Symbol")
        if raw_symbol:
        ...
        times = chart.get("t", []) or []
        opens = chart.get("o", []) or []
        highs = chart.get("h", []) or []
        lows = chart.get("l", []) or []
        closes = chart.get("c", []) or []
        volumes = chart.get("v", []) or []
        values = chart.get("accumulatedValue", []) or []
