"""System prompt + user-content builder for the BCTC dashboard narrative layer.

``SYSTEM_PROMPT`` is the verbatim content of
``docs/superpowers/specs/bctc-dashboard/SYSTEM_PROMPT_ai_generate.md`` embedded
as a constant (the file is the source of truth; keep the two in sync).

``build_user_content(data)`` flattens a ``BctcDashboardData`` dict (produced by
:func:`app.services.bctc_dashboard.compute.compute_dashboard`) into the compact
JSON payload the model reads — ticker/company/template/price/fair_value plus a
flattened ``metrics`` map and the full ``blocks`` (series/history/waterfall) so
the model has every number it may reference without ever computing one.
"""
...
# ─────────────────────────────────────────────────────────────────────────────
# SYSTEM PROMPT — verbatim copy of SYSTEM_PROMPT_ai_generate.md
# (source of truth: backend/docs/superpowers/specs/bctc-dashboard/…)
# ─────────────────────────────────────────────────────────────────────────────

SYSTEM_PROMPT = r"""# SYSTEM PROMPT — AI sinh nội dung Dashboard Phân tích BCTC
...
def _series_history(series: Any, value_key: str = "value") -> list[Any] | None:
    """Extract a plain ``[v0, v1, ...]`` history list from a block series."""
    if not isinstance(series, list):
        return None
    out = [row.get(value_key) for row in series if isinstance(row, dict)]
    return out or None


def _flatten_metrics(blocks: dict[str, Any]) -> dict[str, dict[str, Any]]:
    """Flatten every block's ``metrics`` list into a ``{key: {...}}`` map.

    Each entry keeps value/unit/peer_median/color and — where a matching block
    series exists — a ``history`` list, so the model can reference the 5-year
    trend without ever recomputing it.
    """
    ...
    # Attach histories from well-known series where the block carries them.
    def _attach(metric_key: str, series: Any, value_key: str = "value") -> None:
        if metric_key in flat:
            hist = _series_history(series, value_key)
            if hist is not None:
                flat[metric_key]["history"] = hist

    business = blocks.get("business") or blocks.get("earning") or {}
    if isinstance(business, dict):
        _attach("revenue_growth", business.get("revenue_series"), "revenue")
        _attach("nim", business.get("nim_series"), "nim")
    return flat


def build_user_content(data: dict[str, Any]) -> str:
    """Build the JSON user-content string fed to the model.

    Defensive: tolerates partial/empty dashboards (all fields default to
    ``None``) so a degraded compute never breaks the narrative call.
    """
    data = data or {}
    ...
    payload = {
        ...
        "company_name": hero.get("name"),
        "template": data.get("template"),
        "sub_sector": data.get("sub_sector"),
        "price": hero.get("price"),
        "fair_value": hero.get("fair_value"),
        "upside_pct": hero.get("upside_pct"),
        "radar": [
            {
                "key": d.get("key"),
                "label": d.get("label"),
            ...
            for d in (radar.get("dims") or [])
            if isinstance(d, dict)
        ],
        "metrics": _flatten_metrics(blocks),
        # Full deterministic blocks (series/waterfall/checklist/peer) so the model
        # has every number to reference; it must not compute new ones.
        "blocks": blocks,
        "is_estimated_fields": meta.get("is_estimated_fields") or [],
        "disclaimers": meta.get("disclaimers") or [],
    }
